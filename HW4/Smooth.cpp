...
int main(int argc, char *argv[])
{
    ...
    int start[numThread], finish[numThread];
    int elements_per_thread = bmpInfo.biHeight / numThread;
    int rem = bmpInfo.biHeight % numThread;
    for (int i = 0; i < numThread; i++)
    {
        start[i] = i * elements_per_thread;
        finish[i] = (i + 1) * elements_per_thread;
    }
    finish[numThread] += rem;

    pthread_t tid[numThread];
    threadParameter *tp[numThread];

    for (int i = 0; i < numThread; i++)
    {
        tp[i] = (threadParameter *)malloc(sizeof(threadParameter));
        tp[i]->start = start[i];
        tp[i]->finish = finish[i];
        pthread_create(&tid[i], NULL, smooth, (void *)tp[i]);
    }
...
void *smooth(void *arg)
{
    threadParameter *tp = (threadParameter *)arg;
    // 進行多次的平滑運算
    for (int count = 0; count < NSmooth; count++)
    {
        ...
        // 進行平滑運算
        for (int i = tp->start; i < tp->finish; i++)
        {
        ...
        pthread_mutex_lock(&mutex);
        if (counter[count % 2] == numThread - 1)
        {
            counter[(count + 1) % 2] = 0;
            swapflag = false;
        }
        counter[count % 2]++;
        pthread_mutex_unlock(&mutex);
        while (counter[count % 2] < numThread)
            ;
    }
    return NULL;
}
